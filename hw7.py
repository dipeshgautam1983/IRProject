...
from indexer import invertedindex,pageranker
...
print("HW7")
userinput = input('enter "1" for preprocessing or "2" for indexing or "3" for crawling or 4 for ranking: ')
if userinput.lower() == "1":
    ...
    invertedindex.createinvertedindex(settings.__preprocessed_dir__, postinglist=globals.getposting(),
                                      doclensqrlist=globals.getdoclensqrlist(),docurlmap=globals.getdocurlmap())
    #print(globals.getposting())
    print("Retrieved documents with rank score: \n")
    list = pageranker.getrankeddocumentlist(globals.getposting(), "class student servic offer test jackson hotel", globals.getdoclensqrlist())
